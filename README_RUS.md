...
Реализация собственной библиотеки nd_decimal.h.
...
| ------ | ------ |------------------------------------------------------------------------------------|
| Сложение | + | int nd_add(nd_decimal value_1, nd_decimal value_2, nd_decimal *result)         |
| Вычитание | - | int nd_sub(nd_decimal value_1, nd_decimal value_2, nd_decimal *result)         |
| Умножение | * | int nd_mul(nd_decimal value_1, nd_decimal value_2, nd_decimal *result) | 
| Деление | / | int nd_div(nd_decimal value_1, nd_decimal value_2, nd_decimal *result) |
| Остаток от деления | Mod | int nd_mod(nd_decimal value_1, nd_decimal value_2, nd_decimal *result) |
...
| Название оператора | Оператор  | Функция | 
| ------ | ------ | ------ |
| Меньше  | < | int nd_is_less(nd_decimal, nd_decimal) |
| Меньше или равно | <= | int nd_is_less_or_equal(nd_decimal, nd_decimal) | 
| Больше | \> |  int nd_is_greater(nd_decimal, nd_decimal) |
| Больше или равно | \>= | int nd_is_greater_or_equal(nd_decimal, nd_decimal) | 
| Равно | == |  int nd_is_equal(nd_decimal, nd_decimal) |
| Не равно | != |  int nd_is_not_equal(nd_decimal, nd_decimal) |
...
| ------ | ------ |
| Из int | int nd_from_int_to_decimal(int src, nd_decimal *dst) |
| Из float  | int nd_from_float_to_decimal(float src, nd_decimal *dst) |
| В int  | int nd_from_decimal_to_int(nd_decimal src, int *dst) |
| В float  | int nd_from_decimal_to_float(nd_decimal src, float *dst) |
...
| Описание | Функция                                                  | 
| ------ |----------------------------------------------------------|
| Округляет указанное Decimal число до ближайшего целого числа в сторону отрицательной бесконечности. | int nd_floor(nd_decimal value, nd_decimal *result)    |	
| Округляет Decimal до ближайшего целого числа. | int nd_round(nd_decimal value, nd_decimal *result)    |
| Возвращает целые цифры указанного Decimal числа; любые дробные цифры отбрасываются, включая конечные нули. | int nd_truncate(nd_decimal value, nd_decimal *result) |
| Возвращает результат умножения указанного Decimal на -1. | int nd_negate(nd_decimal value, nd_decimal *result)   |
...
- Оформить решение как статическую библиотеку (с заголовочным файлом nd_decimal.h)
- Библиотека должна быть разработана в соответствии с принципами структурного программирования
- Перед каждой функцией использовать префикс nd_
- Подготовить полное покрытие unit-тестами функций библиотеки c помощью библиотеки Check  
- Unit-тесты должны покрывать не менее 80% каждой функции  
- Предусмотреть Makefile для сборки библиотеки и тестов (с целями all, clean, test, nd_decimal.a, gcov_report)  
...
- Конечные нули можно как оставлять, так и удалять (за исключением функции `nd_truncate`)
